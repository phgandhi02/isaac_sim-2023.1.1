...
class PhysicsBaseAsyncTestCase(omni.kit.test.AsyncTestCase, utils.AssertTestUtils):
    ...
    def is_vp2(self):
        if self.is_vp2_cache is None:
            import omni.kit.viewport.utility as viewport_utils
            viewport_api = viewport_utils.get_active_viewport()
            self.is_vp2_cache = viewport_api and not hasattr(viewport_api, 'legacy_window')
        return self.is_vp2_cache
    ...
    def subTest(self, subtest_name=None, test_name=None, **params):
        ...
        if self.is_vp2():
            full_name = f"viewport_next+{full_name}"
