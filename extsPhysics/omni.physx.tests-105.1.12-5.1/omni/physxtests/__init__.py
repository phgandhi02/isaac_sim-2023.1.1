...
class Window(ui.Window):
    ...
    def __init__(self):
        ...
        def is_phys_module(ext_id):
            ...
            if config["path"].find("extsphysics") != -1:
                return package["name"]
        ...
        def on_ext_disabled(ext_id, *_):
            ret = self._test_modules.pop(ext_id, None)
            if ret is not None:
                self._build_ui()
    ...
    def _run_cpp_tests(self):
        ...
        p = subprocess.Popen([f'{kit_folder}\\..\\test.unit.physics.bat'], universal_newlines=True, stdout=subprocess.PIPE)
    ...
    def _run_cpp_tests_async(self):
        ...
        kit_folder = carb.tokens.get_tokens_interface().resolve("${kit}")
        p = subprocess.Popen([f'{kit_folder}\\..\\test.unit.physics.bat', '--tc="*CCT*'], universal_newlines=True, stdout=subprocess.PIPE)

        def enqueue_output(out, queue):
            try:
                for line in iter(out.readline, b''):
                    if p.poll() is None and line != '' and line.startswith("[d"):
...
class Extension(omni.ext.IExt): 
    def on_startup(self):
        self._window = WindowMenuItem("Test Runner", lambda: Window(), carb.settings.get_settings().get("physics/developmentMode"))
