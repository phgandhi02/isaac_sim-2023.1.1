...
class PhysxInterfaceBackwardsCompatibilityTestKitStage(PhysicsKitStageAsyncTestCase):
    ...
    async def test_backward_compat_articulation_force_sensor(self):
        ...
        cube = physicsUtils.add_cube(stage, self.default_prim_path.AppendChild("cube"), 20.0)
        UsdPhysics.RigidBodyAPI.Apply(cube.GetPrim())
        UsdPhysics.CollisionAPI.Apply(cube.GetPrim())
        cube.GetPrim().AddAppliedSchema("PhysxArticulationForceSensorAPI")

        # check that we got a compatibility warning
        message = f"Physics backwardsCompatibility: PhysxArticulationForceSensorAPI is deprecated and will be removed in a future version. Prim ({cube.GetPath()})."
