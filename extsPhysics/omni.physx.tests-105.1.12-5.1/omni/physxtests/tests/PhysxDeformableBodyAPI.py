...
class PhysxDeformableBodyAPITestAsync(PhysicsBaseAsyncTestCase):
    ...
    async def _run_recooking_test(self, change_type: str, sync_type: str):
        ...
        for mesh in mesh_list:
            ...
            sb_crc_attr = mesh.GetPrim().GetAttribute("physxDeformable:softBodyCrc")
            self.assertTrue(tetmesh_crc_attr)
            self.assertTrue(sb_crc_attr)

            #store initial_crc
            initial_tetmesh_crc_list.append(tetmesh_crc_attr.Get())
            initial_sb_crc_list.append(sb_crc_attr.Get())
        ...
        # check expected CRCs
        for mesh, initial_sb_crc, initial_tetmesh_crc in zip(mesh_list, initial_sb_crc_list, initial_tetmesh_crc_list):
            tetmesh_crc_attr = mesh.GetPrim().GetAttribute("physxDeformable:tetMeshCrc")
            sb_crc_attr = mesh.GetPrim().GetAttribute("physxDeformable:softBodyCrc")
            self.assertTrue(tetmesh_crc_attr)
            self.assertTrue(sb_crc_attr)
            ...
            if change_type in ["change_translation", "change_rotation", "change_scale_uniform"]:
                self.assertTrue(initial_sb_crc == sb_crc_attr.Get())
    ...
    async def test_softbody_trigger_recooking_mesh_during_play(self):
        ...
        error_message = 'Changing deformable body simulation mesh cooking parameter during simulation is not supported.'
    ...
    def _check_failed_cooking_state_deformable_body(self, mesh):
        ...
        self.assertTrue(len(deformableBody.GetCollisionIndicesAttr().Get()) == 0)
        self.assertTrue(len(deformableBody.GetCollisionRestPointsAttr().Get()) == 0)
        self.assertTrue(len(PhysxSchema.PhysxDeformableAPI(deformableBody).GetSimulationIndicesAttr().Get()) == 0)
        self.assertTrue(len(deformableBody.GetSimulationRestPointsAttr().Get()) == 0)
    ...
    async def test_deformable_body_restpoints(self):
        ...
        #clear the rest points so that it is different in size compared to the mesh points
        rest_points = []
