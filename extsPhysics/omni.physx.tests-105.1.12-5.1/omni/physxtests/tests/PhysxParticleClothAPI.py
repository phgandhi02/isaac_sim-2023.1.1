...
class PhysxParticleClothAPITestAsync(PhysicsBaseAsyncTestCase):
    ...
    async def test_particle_cloth_trigger_recooking_mesh_during_play(self):
        ...
        error_message = 'Changing particle cloth mesh or PhysxSchemaPhysxAutoParticleClothAPI parameter during simulation is not supported.'
    ...
    async def test_particle_cloth_invalid_mesh(self):
        ...
        cloth_prim_1 = usd_context.get_stage().GetPrimAtPath(Sdf.Path("/World/RootNode/Katherine_bag/Katherine_bag"))
        particle_cloth_api_1 = PhysxSchema.PhysxParticleClothAPI(cloth_prim_1)
        particle_cloth_api_1.GetPressureAttr().Set(0.0)

        cloth_prim_2 = usd_context.get_stage().GetPrimAtPath(Sdf.Path("/World/RootNode/Katherine_bag_01/Katherine_bag"))
