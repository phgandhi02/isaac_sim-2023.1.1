...
def get_default_particle_system(stage: Usd.Stage) -> PhysxSchema.PhysxParticleSystem:
    ...
    particle_system_path = Sdf.Path(
        omni.usd.get_stage_next_free_path(stage, str(get_default_particle_system_path(stage)), True)
    )
...
def AddPBDMaterialWater(p):
    ...
    execute("AddPBDMaterial", stage=p.GetStage(), path=p.GetPath(), cohesion=0.01, drag=0, lift=0, damping=0, friction=0.1, surface_tension=0.0074*mpu*mpu*mpu, viscosity=0.0000017/(mpu*mpu*mpu), vorticity_confinement=0)

def AddPBDMaterialViscous(p):
    mpu = UsdGeom.GetStageMetersPerUnit(p.GetStage())
    execute("AddPBDMaterial", stage=p.GetStage(), path=p.GetPath(), cohesion=0.2, drag=0.1, lift=0.5, damping=0, friction=0.2, surface_tension=0.1*mpu*mpu*mpu, viscosity=0.0017/(mpu*mpu*mpu), vorticity_confinement=0)
...
def add_physx_particle_cloth_with_constraints(
    ...
    cloth_api.CreateParticleSystemRel().SetTargets([particle_system_path])
