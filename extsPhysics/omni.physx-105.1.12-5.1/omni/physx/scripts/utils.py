...
def setCollider(prim, approximationShape="none", custom_execute_fn=None):
    ...
    if prim.IsA(UsdGeom.Mesh) or prim.IsInstanceable():
        api = MESH_APPROXIMATIONS.get(approximationShape, None)
        if api is None:
...
class ExpectMessage():
    def __init__(self, test_case, expected_message, expected_result=True):
    ...
    def __enter__(self):
        ...
        expt = ("Expecting" if self._expected_result else "Not expecting") + " error message: "
        print(f"{expt} '{self._expected_message}'.")
        self._test_case.fail = partial(fail, self._test_case.fail)
        get_physxunittests_interface().start_check_for_error(self._expected_message)

    def __exit__(self, type, value, traceback):
        found = get_physxunittests_interface().end_check_for_error()
        res = found == self._expected_result
