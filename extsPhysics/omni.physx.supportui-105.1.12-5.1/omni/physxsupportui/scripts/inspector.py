...
class PhysXInspector:
    def __init__(self):
        ...
        self._settings_inspector_enabled_sub = omni.kit.app.SettingChangeSubscription(
            pxsupportui.SETTINGS_PHYSICS_INSPECTOR_ENABLED, self._physics_inspector_enabled_setting_changed
        )
    ...
    def _enable_inspector(self):
        ...
        self.add_inspector_window(works_on_selection=True)
    ...
    def add_inspector_window(self, works_on_selection=False):
        model = PhysXInspectorModel([])
        self._stable_window_id = self._stable_window_id + 1
        new_window = PhysXInspectorWindow(  title_prefix="Physics Inspector: ",
                                            title_stable_id=f"PhysicsInspector{self._stable_window_id}",
                                            model=model, inspector=self, works_on_selection=works_on_selection)
        self._inspector_windows.append(new_window)

    def lock_all_inspector_windows(self):
        for inspector_window in self._inspector_windows:
            inspector_window._model_inspector.get_is_locked_model().set_value(True)
