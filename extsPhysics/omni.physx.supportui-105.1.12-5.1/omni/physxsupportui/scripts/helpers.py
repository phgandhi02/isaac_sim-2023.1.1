...
class Helpers:
    ...
    def create_static_coll_menu():
        ...
        for k, v in Helpers._static_coll_approximations.items():
            Helpers.create_settings_menu_item_int(v, pxsupportui.SETTINGS_STATIC_COLLIDER_SIMPLIFICATION_TYPE, k)

    @staticmethod
    def create_dynamic_coll_menu():
        ui.Separator('Dynamic Collider Approximation'),
        for k, v in Helpers._dynamic_coll_approximations.items():
            Helpers.create_settings_menu_item_int(v, pxsupportui.SETTINGS_DYNAMIC_COLLIDER_SIMPLIFICATION_TYPE, k)
