...
class PhysxSupportUiExtension(omni.ext.IExt):
    def on_startup(self, ext_id):
        omni.ui.Workspace.set_show_window_fn(ActionBar.get_toolbar_name(), partial(self._on_main_menu_click, None))
        ...
        menu_name = "Physics Authoring Toolbar"
        self._menu_path = f"Window/Physics/{menu_name}"
        create_action_bar = self._settings.get(pxsupportui.SETTINGS_ACTION_BAR_ENABLED)
        self._menu = omni.kit.ui.get_editor_menu().add_item(self._menu_path, self._on_main_menu_click, True, value=create_action_bar)
        self._inspector = PhysXInspector()
    ...
    def show_action_bar(self, visible):
        ...
        self._on_action_bar_visiblity_changed(visible)

    def create_action_bar(self):
        self._action_bar = ActionBar(self._icons_folder, self._physx_supportui_interface)
        if self._action_bar:
            self._action_bar.set_action_bar_visibility_changed_fn(self._on_action_bar_visiblity_changed)
    ...
    def _on_action_bar_visiblity_changed(self, visible):
        if self._menu:
            menu_enabled = omni.kit.ui.get_editor_menu().get_value(self._menu_path)
            if (menu_enabled and not visible) or (not menu_enabled and visible):                        
    ...
    def _on_main_menu_click(self, menu, toggled):
        self._settings.set_bool(pxsupportui.SETTINGS_ACTION_BAR_ENABLED, toggled)
