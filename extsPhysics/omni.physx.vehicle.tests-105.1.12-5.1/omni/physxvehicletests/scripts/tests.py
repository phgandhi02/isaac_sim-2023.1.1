...
class PhysXVehicleTestKitStage(PhysicsKitStageAsyncTestCase, PhysXVehicleTestBase):
    ...
    async def test_ogn_get_drive_and_wheel_state(self):
        ...
        from omni.physxgraph.ogn.OgnVehicleGetDriveStateDatabase import OgnVehicleGetDriveStateDatabase
        from omni.physxgraph.ogn.OgnVehicleGetWheelStateDatabase import OgnVehicleGetWheelStateDatabase
        ...
        (graph, nodes, _, _) = ogController.edit(
            ...
            },
            {
                ogController.Keys.CREATE_NODES: [
                    ("driveState", "omni.physx.graph.VehicleGetDriveState"),
                    ("wheelState", "omni.physx.graph.VehicleGetWheelState")
                ],
                ogController.Keys.SET_VALUES: [
                    ("driveState." + OgnVehicleGetDriveStateDatabase.INTERFACE.inputs.vehiclePath.name, vehiclePaths[0]),
                    ("wheelState." + OgnVehicleGetWheelStateDatabase.INTERFACE.inputs.vehicleWheelAttachmentPaths.name, wheelAttachmentPaths[0])
                ]
            }
        )
        ...
        # note: OgnVehicleGetDriveStateDatabase.INTERFACE is something that is likely to change
        #       in the future. In case there will be no alternative, the value ID has to be hardcoded
        #       to "driveState.inputs:vehiclePath"
        
        driveStateNode = OgnVehicleGetDriveStateDatabase(nodes[0])
        wheelStateNode = OgnVehicleGetWheelStateDatabase(nodes[1])
        ...
        for i in range(0, 3):
            ...
            nodes[0].set_compute_incomplete()
            nodes[1].set_compute_incomplete()
            #nodes[0].request_compute()  #!!! this is supposed to be the right thing to use but does not work
            #nodes[1].request_compute()  #!!! this is supposed to be the right thing to use but does not work
            ...
            currentEngSpeed = driveStateNode.outputs.engineRotationSpeed
            ...
            currentAccelerator = driveStateNode.outputs.accelerator
            ...
            for wheelId in range(0, 4):
                ...
                currentWheelRotationSpeed = wheelStateNode.outputs.wheelRotationSpeeds[wheelId];
                ...
                currentWheelRotationAngle = wheelStateNode.outputs.wheelRotationAngles[wheelId]
                ...
                currentWheelSteerAngle = wheelStateNode.outputs.wheelSteerAngles[wheelId]
                ...
                currentIsOnGround = wheelStateNode.outputs.groundContactStates[wheelId]
                refIsOnGround = wheelState[VEHICLE_WHEEL_STATE_IS_ON_GROUND]
                self.assertEqual(currentIsOnGround, refIsOnGround)

                currentGroundPlane = wheelStateNode.outputs.groundPlanes[wheelId];
                ...
                currentGroundPos = wheelStateNode.outputs.groundHitPositions[wheelId]
                ...
                currentGroundPhysXActor = wheelStateNode.outputs.groundPhysXActors[wheelId]
                ...
                currentGroundPhysXShape = wheelStateNode.outputs.groundPhysXShapes[wheelId]
                ...
                currentGroundPhysXMaterial = wheelStateNode.outputs.groundPhysXMaterials[wheelId]
                ...
                currentSuspensionJounce = wheelStateNode.outputs.suspensionJounces[wheelId]
                ...
                currentSuspensionForce = wheelStateNode.outputs.suspensionForces[wheelId]
                ...
                currentTireFriction = wheelStateNode.outputs.tireFrictions[wheelId]
                ...
                currentTireLongSlip = wheelStateNode.outputs.tireLongitudinalSlips[wheelId]
                ...
                currentTireLatSlip = wheelStateNode.outputs.tireLateralSlips[wheelId]
                ...
                currentTireLongDir = wheelStateNode.outputs.tireLongitudinalDirections[wheelId]
                ...
                currentTireLatDir = wheelStateNode.outputs.tireLateralDirections[wheelId]
                ...
                currentTirForce = wheelStateNode.outputs.tireForces[wheelId];
        ...
        for j in range(2):
            ...
            for i in range(2):
                ...
                if (testAtRuntime):
                    ...
                    # to avoid multiple error messages since only one can be tested for at the moment.
                    # It would be important to test for both error messages in the future.
                    vehiclePrim.RemoveAPI(PhysxSchema.PhysxVehicleControllerAPI)
                    ...
                    # the following error message is expected too but can't yet expect more than one message
