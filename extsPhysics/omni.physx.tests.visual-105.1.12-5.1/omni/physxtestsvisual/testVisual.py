...
class PhysxVisualTest(TestCase):
    ...
    async def test_physics_visual_joint_selection(self):
        ...
        revoluteJoint.CreateBody0Rel().SetTargets(val1)
        revoluteJoint.CreateBody1Rel().SetTargets(val2)
        revoluteJoint.CreateLocalPos0Attr().Set(Gf.Vec3f(0.0, 4.0, 0.0))
        revoluteJoint.CreateLocalPos1Attr().Set(Gf.Vec3f(0.0, -4.0, 0.0))

        await omni.kit.app.get_app().next_update_async()

        usdContext = omni.usd.get_context()
        usdContext.get_selection().set_selected_prim_paths(["/revoluteJoint"], True) 
