...
class SimulationView:
    ...
    def create_articulation_view(self, pattern, enable_dof_force_sensors=False):
        return ArticulationView(self._backend.create_articulation_view(pattern, enable_dof_force_sensors), self._frontend)
...
class ArticulationView:
    ...
    @property
    def max_force_sensors(self):
        carb.log_warn("DEPRECATED: max_force_sensors is deprecated. Use get_link_incoming_joint_force() to obtain per-link forces instead.")
        return self._backend.max_force_sensors
    ...
    def enable_dof_force_sensors(self):
        carb.log_warn("DEPRECATED: enable_dof_force_sensors() is deprecated. Use get_link_incoming_joint_force() to obtain per-link forces instead.")
        if not self._backend.enable_dof_force_sensors():
            raise Exception("Failed to enable DOF force sensors from backend")
    ...
    def get_dof_forces(self):
        carb.log_warn("DEPRECATED: get_dof_forces() is deprecated. Use get_link_incoming_joint_force() to obtain per-link forces instead.")
        if not hasattr(self, "_dof_forces"):
            self._dof_forces, self._dof_forces_desc = self._frontend.create_tensor(
                (self.count, self.max_dofs), float32
            )

        if not self._backend.get_dof_forces(self._dof_forces_desc):
            raise Exception("Failed to get DOF forces from backend")
    ...
    def get_force_sensor_forces(self):
        carb.log_warn("DEPRECATED: get_force_sensor_forces() is deprecated. Use get_link_incoming_joint_force() to obtain per-link forces instead.")
        
        if not self.max_force_sensors:
            raise Exception("Articulation view contains no force sensors")
        
        if not hasattr(self, "_force_sensor_forces"):
            self._force_sensor_forces, self._force_sensor_forces_desc = self._frontend.create_tensor(
                (self.count, self.max_force_sensors, 6), float32
            )

        if not self._backend.get_force_sensor_forces(self._force_sensor_forces_desc):
            raise Exception("Failed to get force sensor forces from backend")

        return self._force_sensor_forces
