...
class PhysXCctBaseTests(PhysicsKitStageAsyncTestCase):
    ...
    async def test_cct_velocity(self):
        ...
        def on_stage_update(_, dt):
            ...
            u += 1
            t += dt
            get_physx_cct_interface().set_move(path, (0, 1 * dt, 0))

        stage_update_node = utils.register_stage_update_node("cct_test", on_update_fn=on_stage_update)
        ...
        for ttc in [24, 30, 50, 60]:
            with self.subTest(str(ttc)):
                ...
                print(f"{tran[0]:.4f} in {t:.4f}s in {u} stage updates")
