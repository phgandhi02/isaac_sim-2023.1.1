...
class OgnCharacterController:
    ...
    @staticmethod
    def release(node):
        state = OgnCharacterControllerDatabase.per_node_internal_state(node)
        if state.cct_handle:
    ...
    def activate(db, stage):
        if db.internal_state.cct_handle:
            carb.log_warn("OgnCharacterController: Activating an already activated CharacterController. Cleaning up previous instance.")
            db.internal_state.cct_handle.disable()
            db.internal_state.cct_handle.shutdown()
            db.internal_state.cct_handle = None
        ...
        db.internal_state.cct_handle = cct
        db.outputs.done = og.ExecutionAttributeState.ENABLED
        return True

    @staticmethod
    def deactivate(db):
        cct = db.internal_state.cct_handle
        if not cct:
            db.outputs.done = og.ExecutionAttributeState.ENABLED
            return False
