    ...
    def __init__(self):
        ...
        def on_stage_event(event):
            ...
            if (not self._usd_context.get_stage()):
                return
            # first check if the Stage is a valid OmniPVD Stage
...
class PhysxPvdExtension(omni.ext.IExt):
        ...
        with ui.HStack():
            ...
            with ui.VStack():
                ...
                with ui.HStack():
                    ...
                    self._axisComboBox = omni.ui.ComboBox(self._omniPvdUpAxis, *axisTypes, tooltip="Change the up axis of the Stage", width = 80, height = 20)
    ...
    def _build_usdtform_record(self):
    ...
    def _build_omnipvd_gizmos(self):
        ...
        with ui.HStack():
            ...
            ui.Spacer(width=5)
            create_setting_widget(SETTING_OMNIPVD_GIZMO_GLOBAL_SCALE, SettingType.FLOAT, 0.0, 100.0, width = 80, height = 15)
        ui.Spacer(height=10)
    ...
    def _build_physxusdtform(self):
        ...
        with ui.HStack():
            ...
            #with ui.VStack():
            #    ui.Spacer(height=7)
            #    ui.Label("Is the current Stage an OmniPVD USD Stage?", height = 5)
        ui.Spacer(height=5)
        with ui.HStack():
            ui.Spacer(width=5)
        ...
        with self._window_example.frame:
            with ui.ScrollingFrame(
                ...
                with ui.VStack(height = 0):
                    ...
                    ui.Spacer(height=5)
                    build_section("Record OVD", self._build_usdtform_record)
                    ui.Spacer(height=5)
    ...
    def on_startup(self, ext_id):
        ...
        ################################################################################
        self._menu = WindowMenuItem("OmniPVD", lambda: self._make_my_window(), True)

        ################################################################################
        # Create the OmniPVD Object Tree
        ################################################################################
        self._treeView = WindowMenuItem("OmniPVD Object Tree view", lambda: OmniPVdObjectTreeWindow(), True)
...
loaders.import_tests_auto("omni.physxpvd.scripts.tests")
