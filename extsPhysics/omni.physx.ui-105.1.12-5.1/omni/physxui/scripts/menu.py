...
class PhysicsMenu:
    ...
    def _on_create_attachment(self):
        ...
        if  len(target_paths) < 1 or len(target_paths) > 2:
            carb.log_warn("No attachment was created. One or two prims need to be selected to create an attachment.")
            return

        if len(target_paths) == 1 and len(deformables) == 0:
            carb.log_warn("No attachment was created. One or more deformable or particle cloth or deformable surface prims need to be selected.")
            return

        if len(particlecloths) > 1:
            carb.log_warn("No attachment was created. Attachments between two prims with particle cloth component are not supported.")
            return

        if len(deformablesurfaces) > 1:
            carb.log_warn("No attachment was created. Attachments between two prims with deformable surface component are not supported.")
            return

        if len(hair) > 1:
            carb.log_warn("No attachment was created. Attachments between two prims with hair simulation are not supported.")
            return

        if len(rigids) > 1:
            carb.log_warn("No attachment was created. To connect two rigid bodies use joints.")
            return
