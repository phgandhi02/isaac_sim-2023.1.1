...
def remove_schema(physx_schema_remove, physics_schema_remove, stage, paths):
    def removeAPI(prim, api_name):
        ...
        if not primSpec:
            ...
            targetPath = editTarget.MapToSpecPath(prim.GetPath())
            if targetPath.IsEmpty():
                primSpec = Sdf.PrimSpec()
            else:
                primSpec = Sdf.CreatePrimInLayer(editTarget.GetLayer(), targetPath)
...
class PhysxDebugWindow(ui.Window):
    ...
    def setup(self):
        self.vis_names = [
            ...
            ["ContactForce", "Contact Force", False],
        ...
        def build_inner():
            with ui.VStack():
                with ui.HStack(height=LINE_HEIGHT):
                    ui.Label("Robotics", width=LABEL_WIDTH)
                    rob_model = ui.SimpleBoolModel(settings.get(SETTING_LOG_ROBOTICS))
                    rob_model.add_value_changed_fn(lambda val: settings.set(SETTING_LOG_ROBOTICS, val.as_bool))
    ...
    def _build_collision_mesh_visualization_property_ui(self):
        ...
        get_physxui_interface().enable_collision_mesh_visualization(cl_mesh_default)

        def cb_cl_mesh_value_changed(value):
            carb.settings.get_settings().set(SETTING_VISUALIZATION_COLLISION_MESH, value.as_bool)
            get_physxui_interface().enable_collision_mesh_visualization(value.as_bool)
