"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldConical

A conical force field that attracts and/or repels rigid bodies from a central point depending on the function coefficients.
Positive values attract and negative values repel. The net force on the rigid body is calculated using f = constant + linear
* r + inverseSquare / r^2, where r is the distance to the center. Forces can be attenuated based on their position relative
to the cone central axis with the  linearFalloff and powerFalloff inputs.
"""
...
class OgnForceFieldConicalDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldConical
    ...
    INTERFACE = og.Database._get_interface([
        ('inputs:angle', 'float', 0, None, 'Angle from the central axis of the cone to the surface that defines the extents of the cone.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ('inputs:constant', 'float', 0, None, 'constant applies a steady force.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ('inputs:direction', 'normal3f', 0, None, 'The direction vector of the central axis of the cone.', {}, True, [0.0, 0.0, 0.0], False, ''),
        ...
        ('inputs:linearFalloff', 'float', 0, None, 'ramp down forces linearly with the angle from the central axis. set linearFalloff to 0.0 to disable.\nlinearFalloff is the percentage of the force remaining at the cone angle extents (>0.0 to 1.0).', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ...
        ('inputs:powerFalloff', 'float', 0, None, 'ramp down forces dramatically with the angle from the central axis.\nset powerFalloff to 0.0 to disable. >0.0 to 1.0: decay more as this angle increases. 1.0 and greater: decay\nmore as the angle apporaches the cone angle limit.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.direction = og.AttributeRole.NORMAL
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        @property
        def angle(self):
            data_view = og.AttributeValueHelper(self._attributes.angle)
            return data_view.get()

        @angle.setter
        def angle(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.angle)
            data_view = og.AttributeValueHelper(self._attributes.angle)
            data_view.set(value)
        ...
        def linearFalloff(self):
            data_view = og.AttributeValueHelper(self._attributes.linearFalloff)
            return data_view.get()

        @linearFalloff.setter
        def linearFalloff(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.linearFalloff)
            data_view = og.AttributeValueHelper(self._attributes.linearFalloff)
            data_view.set(value)
        ...
        @property
        def powerFalloff(self):
            data_view = og.AttributeValueHelper(self._attributes.powerFalloff)
            return data_view.get()

        @powerFalloff.setter
        def powerFalloff(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.powerFalloff)
            data_view = og.AttributeValueHelper(self._attributes.powerFalloff)
            data_view.set(value)
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldConicalDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldConicalDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldConicalDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
