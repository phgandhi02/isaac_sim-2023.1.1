"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldPlanar

A planar force field that attracts and/or repels rigid bodies from a plane, defined by a point and normal vector, depending
on the function coefficients. Positive values attract and negative values repel. The net force on the rigid body is calculated
using f = constant + linear * r + inverseSquare / r^2, where r is the distance to the closest point on the plane.
"""
...
class OgnForceFieldPlanarDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldPlanar
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:normal', 'normal3f', 0, None, 'The normal vector to the plane.', {}, True, [0.0, 0.0, 0.0], False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.normal = og.AttributeRole.NORMAL
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        @property
        def normal(self):
            data_view = og.AttributeValueHelper(self._attributes.normal)
            return data_view.get()

        @normal.setter
        def normal(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.normal)
            data_view = og.AttributeValueHelper(self._attributes.normal)
            data_view.set(value)
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldPlanarDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldPlanarDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldPlanarDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
