"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldDrag

A force field that slows rigid bodies by generating a force that is opposite to their velocity direction using the forumula
f = -linear * v - square * v^2.
"""
...
class OgnForceFieldDragDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldDrag
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:linear', 'float', 0, None, 'linear sets a force that varies proportionally with the speed of the rigid body.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ('inputs:minimumSpeed', 'float', 0, None, 'Drag forces are not applied below this minimum speed.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ...
        ('inputs:square', 'float', 0, None, 'square computes a force the varies with the square of the speed of the rigid body.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        def minimumSpeed(self):
            data_view = og.AttributeValueHelper(self._attributes.minimumSpeed)
            return data_view.get()

        @minimumSpeed.setter
        def minimumSpeed(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.minimumSpeed)
            data_view = og.AttributeValueHelper(self._attributes.minimumSpeed)
            data_view.set(value)
        ...
        @property
        def square(self):
            data_view = og.AttributeValueHelper(self._attributes.square)
            return data_view.get()

        @square.setter
        def square(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.square)
            data_view = og.AttributeValueHelper(self._attributes.square)
            data_view.set(value)
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldDragDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldDragDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldDragDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
