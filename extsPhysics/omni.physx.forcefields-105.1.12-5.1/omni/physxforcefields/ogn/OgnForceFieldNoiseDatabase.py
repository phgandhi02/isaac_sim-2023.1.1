"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldNoise
...
class OgnForceFieldNoiseDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldNoise
    ...
    INTERFACE = og.Database._get_interface([
        ('inputs:amplitude', 'vector3f', 0, None, 'The magnitude of the oscillations along each of the three local axes.', {}, True, [0.0, 0.0, 0.0], False, ''),
        ('inputs:drag', 'float', 0, None, 'The drag coefficient determines how quickly the body moves along with the noise. Smaller values cause the body to react more slowly.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ...
        ('inputs:frequency', 'vector3f', 0, None, 'The frequency of the oscillations for each of the three local axes.', {}, True, [0.0, 0.0, 0.0], False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.amplitude = og.AttributeRole.VECTOR
        role_data.inputs.execution = og.AttributeRole.EXECUTION
        role_data.inputs.frequency = og.AttributeRole.VECTOR
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        @property
        def amplitude(self):
            data_view = og.AttributeValueHelper(self._attributes.amplitude)
            return data_view.get()

        @amplitude.setter
        def amplitude(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.amplitude)
            data_view = og.AttributeValueHelper(self._attributes.amplitude)
            data_view.set(value)
        ...
        @property
        def frequency(self):
            data_view = og.AttributeValueHelper(self._attributes.frequency)
            return data_view.get()

        @frequency.setter
        def frequency(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.frequency)
            data_view = og.AttributeValueHelper(self._attributes.frequency)
            data_view.set(value)
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldNoiseDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldNoiseDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldNoiseDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
