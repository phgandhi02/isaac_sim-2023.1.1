"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldRing

A force field that applies forces to rotate rigid bodies around a ring, defined by a normal axis through the center of the
ring and radius from that axis. The two sets of coefficients describe the attraction force to the ring and the tangential
force that rotated bodies around the ring. The net force on the rigid body is calculated using f_ring = constant + linear
* r + inverseSquare / r^2. f_spin = spinConstant + spinLinear * r + spinInverseSquare / r^2.
"""
...
class OgnForceFieldRingDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldRing
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:normalAxis', 'normal3f', 0, None, 'The normal axis through the center of the ring in world coordinates.', {}, True, [0.0, 0.0, 0.0], False, ''),
        ...
        ('inputs:radius', 'float', 0, None, 'The distance from the normal axis to the center of the ring.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ...
        ('inputs:spinConstant', 'float', 0, None, 'spinConstant applies a steady force that rotates bodies around the ring.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ('inputs:spinInverseSquare', 'float', 0, None, 'spinInverseSquare sets a force that rotates bodies around the ring and varies with the reciprocal of the square of the distance to the ring.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ('inputs:spinLinear', 'float', 0, None, 'spinLinear applies a force that rotates bodies around the ring and varies with distance to the ring.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.normalAxis = og.AttributeRole.NORMAL
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        @property
        def normalAxis(self):
            data_view = og.AttributeValueHelper(self._attributes.normalAxis)
            return data_view.get()

        @normalAxis.setter
        def normalAxis(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.normalAxis)
            data_view = og.AttributeValueHelper(self._attributes.normalAxis)
            data_view.set(value)
        ...
        @property
        def spinConstant(self):
            data_view = og.AttributeValueHelper(self._attributes.spinConstant)
            return data_view.get()

        @spinConstant.setter
        def spinConstant(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.spinConstant)
            data_view = og.AttributeValueHelper(self._attributes.spinConstant)
            data_view.set(value)

        @property
        def spinInverseSquare(self):
            data_view = og.AttributeValueHelper(self._attributes.spinInverseSquare)
            return data_view.get()

        @spinInverseSquare.setter
        def spinInverseSquare(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.spinInverseSquare)
            data_view = og.AttributeValueHelper(self._attributes.spinInverseSquare)
            data_view.set(value)

        @property
        def spinLinear(self):
            data_view = og.AttributeValueHelper(self._attributes.spinLinear)
            return data_view.get()

        @spinLinear.setter
        def spinLinear(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.spinLinear)
            data_view = og.AttributeValueHelper(self._attributes.spinLinear)
            data_view.set(value)
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldRingDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldRingDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldRingDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
