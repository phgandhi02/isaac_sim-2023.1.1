"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldSpin

A force field that applies forces to rotate rigid bodies around a line, defined by a spin axis, and varies with the function
coefficients. Positive values rotate clockwise around the spin axis when looking along the axis. The net force on the rigid
body is calculated using f = constant + linear * r + inverseSquare / r^2.
"""
...
class OgnForceFieldSpinDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldSpin
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:spinAxis', 'normal3f', 0, None, 'The axis around which rigid bodies are rotated in world coordinates.', {}, True, [0.0, 0.0, 0.0], False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.spinAxis = og.AttributeRole.NORMAL
        return role_data
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        @property
        def spinAxis(self):
            data_view = og.AttributeValueHelper(self._attributes.spinAxis)
            return data_view.get()

        @spinAxis.setter
        def spinAxis(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.spinAxis)
            data_view = og.AttributeValueHelper(self._attributes.spinAxis)
            data_view.set(value)
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldSpinDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldSpinDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldSpinDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
