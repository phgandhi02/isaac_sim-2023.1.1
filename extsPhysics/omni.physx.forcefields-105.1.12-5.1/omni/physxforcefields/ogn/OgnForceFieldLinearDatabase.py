"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldLinear

A linear force field that attracts and/or repels rigid bodies from a line, defined by a point and direction vector, depending
...
using f = constant + linear * r + inverseSquare / r^2, where r is the distance to the closest point on the line.
"""
...
class OgnForceFieldLinearDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldLinear
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:direction', 'normal3f', 0, None, 'The direction vector of the line.', {}, True, [0.0, 0.0, 0.0], False, ''),
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldLinearDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldLinearDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldLinearDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
