"""Support for simplified access to data on nodes of type omni.physx.forcefields.ForceFieldWind
...
class OgnForceFieldWindDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.physx.forcefields.ForceFieldWind
    ...
    Attribute Value Properties:
        Inputs:
            inputs.averageDirection
            inputs.averageSpeed
            inputs.directionVariation
            inputs.directionVariationFrequency
            inputs.drag
            inputs.enabled
            inputs.execution
            inputs.position
            ...
            inputs.range
            inputs.shape
            inputs.speedVariation
            inputs.speedVariationFrequency
    ...
    INTERFACE = og.Database._get_interface([
        ('inputs:averageDirection', 'normal3f', 0, None, 'The initial direction of the wind in the world coordinate system. Wind direction\n\t\tchanges are added to this base direction.', {}, True, [0.0, 0.0, 0.0], False, ''),
        ('inputs:averageSpeed', 'float', 0, None, 'The average speed of the wind. Speed changes are added to this base speed, but\n\t\tthe net speed can never drop below zero.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ('inputs:directionVariation', 'normal3f', 0, None, 'The magnitude of the random changes of the wind direction in the world coordinate system.', {}, True, [0.0, 0.0, 0.0], False, ''),
        ('inputs:directionVariationFrequency', 'vector3f', 0, None, 'The frequency of the random variations of the wind direction in the world coordinate system.', {}, True, [0.0, 0.0, 0.0], False, ''),
        ('inputs:drag', 'float', 0, None, 'The drag coefficient determines how quickly the body moves along with the wind.\n\t\tSmaller values cause the body to react more slowly.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
        ...
        ('inputs:shape', 'token[]', 0, None, 'Derive the position input from this prim instead.', {}, True, [], False, ''),
        ('inputs:speedVariation', 'float', 0, None, 'The magnitude of the random changes of the wind speed.', {}, True, 0.0, False, ''),
        ('inputs:speedVariationFrequency', 'float', 0, None, 'The frequency of the random changes of the wind speed.', {ogn.MetadataKeys.DEFAULT: '0.0'}, True, 0.0, False, ''),
    ...
    def _populate_role_data(cls):
        ...
        role_data.inputs.averageDirection = og.AttributeRole.NORMAL
        role_data.inputs.directionVariation = og.AttributeRole.NORMAL
        role_data.inputs.directionVariationFrequency = og.AttributeRole.VECTOR
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        ...
        def averageDirection(self):
            data_view = og.AttributeValueHelper(self._attributes.averageDirection)
            return data_view.get()

        @averageDirection.setter
        def averageDirection(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.averageDirection)
            data_view = og.AttributeValueHelper(self._attributes.averageDirection)
            data_view.set(value)

        @property
        def averageSpeed(self):
            data_view = og.AttributeValueHelper(self._attributes.averageSpeed)
            return data_view.get()

        @averageSpeed.setter
        def averageSpeed(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.averageSpeed)
            data_view = og.AttributeValueHelper(self._attributes.averageSpeed)
            data_view.set(value)

        @property
        def directionVariation(self):
            data_view = og.AttributeValueHelper(self._attributes.directionVariation)
            return data_view.get()

        @directionVariation.setter
        def directionVariation(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.directionVariation)
            data_view = og.AttributeValueHelper(self._attributes.directionVariation)
            data_view.set(value)

        @property
        def directionVariationFrequency(self):
            data_view = og.AttributeValueHelper(self._attributes.directionVariationFrequency)
            return data_view.get()

        @directionVariationFrequency.setter
        def directionVariationFrequency(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.directionVariationFrequency)
            data_view = og.AttributeValueHelper(self._attributes.directionVariationFrequency)
            data_view.set(value)

        @property
        def drag(self):
            data_view = og.AttributeValueHelper(self._attributes.drag)
            return data_view.get()

        @drag.setter
        def drag(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.drag)
            data_view = og.AttributeValueHelper(self._attributes.drag)
            data_view.set(value)
        ...
        @property
        def speedVariation(self):
            data_view = og.AttributeValueHelper(self._attributes.speedVariation)
            return data_view.get()

        @speedVariation.setter
        def speedVariation(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.speedVariation)
            data_view = og.AttributeValueHelper(self._attributes.speedVariation)
            data_view.set(value)

        @property
        def speedVariationFrequency(self):
            data_view = og.AttributeValueHelper(self._attributes.speedVariationFrequency)
            return data_view.get()

        @speedVariationFrequency.setter
        def speedVariationFrequency(self, value):
            if self._setting_locked:
                raise og.ReadOnlyError(self._attributes.speedVariationFrequency)
            data_view = og.AttributeValueHelper(self._attributes.speedVariationFrequency)
            data_view.set(value)
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnForceFieldWindDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnForceFieldWindDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnForceFieldWindDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
