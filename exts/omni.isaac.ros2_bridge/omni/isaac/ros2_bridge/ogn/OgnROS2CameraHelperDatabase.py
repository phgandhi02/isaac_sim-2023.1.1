...
class OgnROS2CameraHelperDatabase(og.Database):
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:frameId', 'string', 0, None, 'FrameId for ROS2 messagem, the nodeNamespace will not be prefixed to the frame id', {ogn.MetadataKeys.DEFAULT: '"sim_camera"'}, True, "sim_camera", False, ''),
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"context", "enableSemanticLabels", "enabled", "execIn", "frameId", "nodeNamespace", "queueSize", "renderProductPath", "resetSimulationTimeOnStop", "semanticLabelsTopicName", "stereoOffset", "topicName", "type", "viewport", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.context, self._attributes.enableSemanticLabels, self._attributes.enabled, self._attributes.execIn, self._attributes.frameId, self._attributes.nodeNamespace, self._attributes.queueSize, self._attributes.renderProductPath, self._attributes.resetSimulationTimeOnStop, self._attributes.semanticLabelsTopicName, self._attributes.stereoOffset, self._attributes.topicName, self._attributes.type, self._attributes.viewport]
            self._batchedReadValues = [0, False, True, None, "sim_camera", "", 10, "", False, "semantic_labels", [0, 0], "rgb", "rgb", ""]
    ...
    class abi:
        ...
        def release_instance(node, target):
            OgnROS2CameraHelperDatabase._release_per_node_instance_data(node, target)
