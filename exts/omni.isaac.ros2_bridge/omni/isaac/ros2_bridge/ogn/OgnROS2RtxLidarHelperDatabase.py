...
class OgnROS2RtxLidarHelperDatabase(og.Database):
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"context", "enabled", "execIn", "frameId", "fullScan", "nodeNamespace", "queueSize", "renderProductPath", "resetSimulationTimeOnStop", "topicName", "type", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.context, self._attributes.enabled, self._attributes.execIn, self._attributes.frameId, self._attributes.fullScan, self._attributes.nodeNamespace, self._attributes.queueSize, self._attributes.renderProductPath, self._attributes.resetSimulationTimeOnStop, self._attributes.topicName, self._attributes.type]
            self._batchedReadValues = [0, True, None, "sim_lidar", False, "", 10, "", False, "scan", "laser_scan"]
    ...
    class abi:
        ...
        def release_instance(node, target):
            OgnROS2RtxLidarHelperDatabase._release_per_node_instance_data(node, target)
